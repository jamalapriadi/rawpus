...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading">Data Role # {{$role->name}}</div>
...
@push('extra-script')
    <script>
        $(function(){
            ...
            function showData(){
                $.ajax({
                    url:"{{URL::to('home/data/permissions')}}?role="+id,
                    type:"GET",
                    beforeSend:function(){
                        $("#divPermissions").empty().html("<div class='alert alert-info'>Please Wait . . .</div>");
                    },
                    success:function(result){
                        var el="";
                        el+="<table class='table table-striped' id='perms'>"+
                            "<thead>"+
                                "<tr>"+
                                    "<th width='5%'>No.</th>"+
                                    "<th width='76%'>Name</th>"+
                            ...
                            "<tbody>";
                                ...
                                if(result.permissions.length>0){
                                    $.each(result.permissions,function(c,d){
                                        no++;
                                        el+="<tr>"+
                                            "<td>"+no+"</td>"+
                                            "<td>"+d.name+"</td>"+
                                            "<td>"+
                                                "<div class='btn-group'>"+
                                                    "<a class='btn btn-sm btn-warning edit' kode='"+d.id+"'><i class='icon-pencil4'></i></a>"+
                                                    "<a class='btn btn-sm btn-danger hapus' kode='"+d.id+"'><i class='icon-trash'></i></a>"+
                                                "</div>"+
                                            "</td>"+
                                        "</tr>";
                                    })
                    ...
                    error:function(){
                        $("#divPermissions").empty().html("<div class='alert alert-danger'>Oppss data failed to load . . .</div>");
                    }
                })
            } 
                        ...
                        '<form id="form" onsubmit="return false;" enctype="multipart/form-data" method="post" accept-charset="utf-8">'+
                            '<div class="modal-content">'+
                                '<div class="modal-header bg-primary">'+
                                    ...
                                    '<h5 class="modal-title" id="modal-title">Add New Permissions</h5>'+
                                '</div>'+
            ...
            $(document).on("submit","#form",function(e){
                ...
                if($("#form")[0].checkValidity()) {
                    ...
                    e.preventDefault();
                    $.ajax({
                        url         : "{{URL::to('home/data/permissions')}}",
                        type        : 'post',
                        data        : data,
                        dataType    : 'JSON',
                        contentType : false,
            ...
            $(document).on("click",".edit",function(){
                ...
                $.ajax({
                    url:"{{URL::to('home/data/permissions')}}/"+kode,
                    type:"GET",
                    beforeSend:function(){
                        el+='<div id="modal_default" class="modal fade" data-backdrop="static" data-keyboard="false">'+
                            '<div class="modal-dialog">'+
                    ...
                    success:function(result){
                        el+='<div id="pesan"></div>'+
                            '<div class="form-group">'+
                                '<label class="control-label text-semibold">Name</label>'+
                                '<input class="form-control" name="name" id="name" placeholder="Name" value="'+result.name+'" required>'+
                            '</div>';
