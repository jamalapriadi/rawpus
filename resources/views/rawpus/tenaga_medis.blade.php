...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4 class="panel-title">Data Tenaga Medis</h4>
        </div>
        ...
        $(function(){
            ...
            function showData(){
                $('.datatable-colvis-basic').DataTable({
                    ...
                    autoWidth: true,
                    destroy: true,
                    ajax: "{{URL::to('home/data/tenaga-medis')}}",
                    columns: [
                        {data: 'no', name: 'no',title:'No.',searchable:false,width:'5%'},
                        {data: 'name', name: 'name',title:'Name'},
                        {data: 'jabatan.name', name: 'jabatan.name',title:'Jabatan',width:'20%'},
                        {data: 'alamat', name: 'alamat',title:'Alamat'},
                        {data: 'no_telp', name: 'no_telp',title:'Telp'},
            ...
            $(document).on("click","#tambah",function(){
                var el="";
                $.ajax({
                    url:"{{URL::to('home/data/list-jabatan')}}",
                    type:"GET",
                    beforeSend:function(){
                    ...
                    success:function(result){
                        el+='<div id="pesan"></div>'+
                            ...
                            "</div>"+
                            '<div class="form-group">'+
                                '<label class="control-label text-semibold">Nama</label>'+
                                '<input class="form-control" name="name" id="name" placeholder="Nama" required>'+
                            '</div>'+
                            '<div class="form-group">'+
                            ...
                            '</div>'+
                            '<div class="form-group">'+
                                '<label class="control-label text-semibold">Telp</label>'+
                                '<input class="form-control" name="telp" id="telp" placeholder="telp" required>'+
                            '</div>';
            ...
            $(document).on("submit","#form",function(e){
                ...
                if($("#form")[0].checkValidity()) {
                    ...
                    e.preventDefault();
                    $.ajax({
                        url			: "{{URL::to('home/data/tenaga-medis')}}",
                        type		: 'post',
                        data		: data,
                        dataType	: 'JSON',
                        contentType	: false,
            ...
            $(document).on("click",".edit",function(){
                ...
                $.ajax({
                    url:"{{URL::to('home/data/tenaga-medis')}}/"+kode,
                    type:"GET",
                    beforeSend:function(){
                        el+='<div id="modal_default" class="modal fade" data-backdrop="static" data-keyboard="false">'+
                            '<div class="modal-dialog">'+
                    ...
                    success:function(result){
                        el+='<div id="pesan"></div>'+
                            '<div class="form-group">'+
                                ...
                                "<select class='form-control' name='jabatan' id='jabatan'>"+
                                    "<option value='' disabled selected>--Pilih Jabatan--</option>";
                                    $.each(result.jabatan,function(a,b){
                                        el+="<option value='"+b.id+"'>"+b.name+"</option>";
                                    })
                                el+="</select>"+
                            "</div>"+
                            '<div class="form-group">'+
                                '<label class="control-label text-semibold">Nama</label>'+
                                '<input class="form-control" name="name" value="'+result.tenaga.name+'" id="name" placeholder="Nama" required>'+
                            '</div>'+
                            '<div class="form-group">'+
                                '<label class="control-label text-semibold">Alamat</label>'+
                                '<input class="form-control" name="alamat" value="'+result.tenaga.alamat+'" id="alamat" placeholder="alamat" required>'+
                            '</div>'+
                            '<div class="form-group">'+
                                '<label class="control-label text-semibold">Telp</label>'+
                                '<input class="form-control" name="telp" id="telp" value="'+result.tenaga.no_telp+'" placeholder="telp" required>'+
                            '</div>';

                        $("#showForm").empty().html(el);

                        $("#jabatan").val(result.tenaga.jabatan_id);
                    },
                    error:function(){
