...
@section('content')
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4 class="panel-title">Status Ketenagaan</h4>
        </div>
        ...
        $(function(){
            ...
            function showData(){
                $('.datatable-colvis-basic').DataTable({
                    ...
                    autoWidth: true,
                    destroy: true,
                    ajax: "{{URL::to('home/data/status-ketenagaan')}}",
                    columns: [
            ...
            $(document).on("submit","#form",function(e){
                ...
                if($("#form")[0].checkValidity()) {
                    ...
                    e.preventDefault();
                    $.ajax({
                        url			: "{{URL::to('home/data/status-ketenagaan')}}",
                        type		: 'post',
                        data		: data,
                        dataType	: 'JSON',
                        contentType	: false,
            ...
            $(document).on("click",".edit",function(){
                ...
                $.ajax({
                    url:"{{URL::to('home/data/status-ketenagaan')}}/"+kode,
                    type:"GET",
                    beforeSend:function(){
                        el+='<div id="modal_default" class="modal fade" data-backdrop="static" data-keyboard="false">'+
                            '<div class="modal-dialog">'+
                    ...
                    success:function(result){
                        el+='<div id="pesan"></div>'+
                            '<div class="form-group">'+
                                '<label class="control-label text-semibold">Name</label>'+
                                '<input class="form-control" name="name" value="'+result.name+'" id="name" placeholder="Name" required>'+
                            '</div>';
