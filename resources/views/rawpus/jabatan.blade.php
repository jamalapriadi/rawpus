        ...
        $(function(){
            ...
            function showData(){
                $('.datatable-colvis-basic').DataTable({
                    ...
                    autoWidth: true,
                    destroy: true,
                    ajax: "{{URL::to('home/data/jabatan')}}",
                    columns: [
                        {data: 'no', name: 'no',title:'No.',searchable:false,width:'5%'},
                        {data: 'status.name', name: 'status.name',title:'Status Ketenagaan',width:'25%'},
            ...
            $(document).on("click","#tambah",function(){
                var el="";
                $.ajax({
                    url:"{{URL::to('home/data/list-status-ketenagaan')}}",
                    type:"GET",
                    beforeSend:function(){
                    ...
                    success:function(result){
                        el+='<div id="pesan"></div>'+
                            '<div class="form-group">'+
                                '<label class="control-label">Status Ketenagaan</label>'+
            ...
            $(document).on("click",".edit",function(){
                ...
                $.ajax({
                    url:"{{URL::to('home/data/jabatan')}}/"+kode,
                    type:"GET",
                    beforeSend:function(){
                        el+='<div id="modal_default" class="modal fade" data-backdrop="static" data-keyboard="false">'+
                            '<div class="modal-dialog">'+
                    ...
                    success:function(result){
                        el+='<div id="pesan"></div>'+
                            '<div class="form-group">'+
                                ...
                                "<select name='status' class='form-control' id='status'>"+
                                    "<option value='' disabled selected>--Pilih Status--</option>";
                                    $.each(result.status,function(a,b){
                                        el+="<option value='"+b.id+"'>"+b.name+"</option>";
                                    })
                                el+="</select>"+
                            "</div>"+
                            '<div class="form-group">'+
                                '<label class="control-label text-semibold">Name</label>'+
                                '<input class="form-control" name="name" value="'+result.jabatan.name+'" id="name" placeholder="Name" required>'+
                            '</div>';

                        $("#showForm").empty().html(el);

                        $("#status").val(result.jabatan.status_ketenagaan_id);
                    },
                    error:function(){
                        $("#showForm").empty().html("<div class='alert alert-danger'>Data Failed to load</div>");
                    }
